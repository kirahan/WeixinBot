...
class PluginManager(object):
    ...
    def load_config(self, groupid):
        ...
        with open('config/plug_config.json',"r") as f:
            try:
                config = json.load(f)[groupid]['plugin_on']
            except:
...
def handlemsg(r):
    for msg in r['AddMsgList']:
            ...
            if msgType == 1:
                ...
                plugin_list = plugmanager.load_config(msg['FromUserName'])
                print plugin_list
                for plug in plugin_list:
                    ...
                    if ans is not None :
                        ...
                        if ans == 'command list plugin':
                            ...
                            status = plugmanager.open_plugin(msg['FromUserName'],plugname)
                            if status:
                            ...
                            plugname = ans.split(':')[1]
                            status = plugmanager.close_plugin(msg['FromUserName'],plugname)
                            if status:
